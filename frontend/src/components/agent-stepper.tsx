...
import { Brain, Cpu, FileText, Loader2, Wrench } from "lucide-react";
...
export function AgentStepper({ active }: { active: string }) {
          ...
          <div key={a} className="flex items-center gap-3">
            <div
              className={cn(
                ...
                state === "active" && "border-primary text-primary glow-primary bg-primary/10",
                state === "pending" && "border-border text-muted-foreground bg-background/30",
              )}
            >
              {state === "active" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Icon className="h-4 w-4" />}
            </div>
